...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndex === -1) {
    return str; // Return the original string if the value is not found
  }

  const beforeSubstring = str.substring(0, lastIndex); // Characters before the substring
  const afterSubstring = str.substring(lastIndex + value.length); // Characters after the substring

  return beforeSubstring + afterSubstring; // Concatenate the parts before and after the substring
}
...
function sumOfCodes(str) {
  if (!str) return 0; // Return 0 if the string is empty or undefined
...
function extractEmails(str) {
  const regex = /[\w.-]+@[a-zA-Z\d.-]+\.[a-zA-Z]{2,}/g;
